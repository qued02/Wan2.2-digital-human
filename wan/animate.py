...
class WanAnimate:
    """
    WanAnimate 视频生成模型的主类。
    负责整合 T5 文本编码器、CLIP 视觉编码器、VAE 和 DiT 扩散模型，
    并实现视频生成流程。
    """
    def __init__(
        self,
        ...
        参数:
            ...
            dit_fsdp (`bool`, *optional*, defaults to False):
                是否对 DiT 模型启用 FSDP 分片
            use_sp (`bool`, *optional*, defaults to False):
                是否启用序列并行的分布式策略
            t5_cpu (`bool`, *optional*, defaults to False):
                是否将 T5 模型放在 CPU 上（仅在不使用 t5_fsdp 时有效）
            init_on_cpu (`bool`, *optional*, defaults to True):
                是否在 CPU 上初始化 Transformer 模型（仅在不使用 FSDP 或 SP 时有效）
            convert_model_dtype (`bool`, *optional*, defaults to False):
                是否将 DiT 模型参数的数据类型转换为 'config.param_dtype'（仅在不使用 FSDP 时有效）
